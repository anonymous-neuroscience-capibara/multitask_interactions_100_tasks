#!/bin/bash
#SBATCH --job-name=cmp_gru
#SBATCH --output=/gs/home/garcias/Projects/multitask_learning/logs/cmp_gru_%A_%a.out
#SBATCH --error=/gs/home/garcias/Projects/multitask_learning/logs/cmp_gru_%A_%a.err
...
# 2026-08-11: hierarchical GRU baseline, joint training on the 100-task battery,
# on x86 CPU (96GBXL, multitask env).
#   M{64,32} x P{8,16} x seed{0,1,2,3,4} = 2 x 2 x 5 = 20 -> --array=0-19
# M=64 is the state-dimension-matched arm; M=32 keeps the same axis as the LSTM
# sweep (the exact params-matched GRU would be M~37; 32 keeps the grids aligned).
# No MAR (no analogue for gated cells; the script forces tau=0). Training config
# matches train_2026-08-06_M64_NPsweep_varnorm_extended_E880.sh (the final_run
# reference): LOSS_VARNORM=1, lr 5e-4, s200/b64, 3000 epochs, patience 300@300,
# warmup 100, w_init_gain 0.05, eval every epoch.
...
OUTDIR="comparisons/results/joint/gru_M${HIDDEN}_P${PARAMS}_s${SAMPLE}_b${BATCH}_lr${BATCH_LR}_seed${SEED}"
...
echo "Config: GRU M=$HIDDEN P=$PARAMS sample=$SAMPLE test=$TEST batch=$BATCH lr=$BATCH_LR epochs=$EPOCHS es=$PATIENCE@$ES_START eval=$EVAL_INTERVAL seed=$SEED  |  LOSS_VARNORM=1"
